...
class VideoResizerClass {
    ...
    start(inputStream: MediaStream, key: string, width: number, height: number | null, fps: number) {
        const self: VideoResizerClass = this;
        // Check if any video track exists
        const videoTrack: MediaStreamTrack = inputStream.getVideoTracks().find((track) => track.enabled);
     ...
     */
    privateGetResizeDimensions(inputStream: MediaStream, requiredWidth: number, requiredHeight: number): VideoDimensions {
        // Get video tracks
